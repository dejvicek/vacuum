...
describe('getQuarterStart', () => {
  ...
  it('throws an error for quarter > 4', () => {
    expect(() => getQuarterStart(2023, 5)).toThrow('Invalid quarter: 5. Quarter must be between 0 and 3.');
  });

  it('throws an error for negative quarter', () => {
    expect(() => getQuarterStart(2023, -2)).toThrow('Invalid quarter: -2. Quarter must be between 0 and 3.');
  });

  it('throws an error for non-integer quarter', () => {
    expect(() => getQuarterStart(2023, 2.5)).toThrow('Invalid quarter: 2.5. Quarter must be between 0 and 3.');
  });
});
